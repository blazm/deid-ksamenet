...
#    model_path = '../de-id/generator/output/{}.h5'.format(model_name)
    ...
    print "Proxy clusters: ", len(clustered_proxys.keys())
    print "Probe clusters: ", len(clustered_probes.keys())
    for i, probe_cluster in enumerate(clustered_probes):
        ...
        proxy_cluster = clustered_proxys[i % (len(clustered_proxys.keys())-1)]  # if there is more probe clusters than proxy clusters, we need to compensate
...
if __name__ == "__main__":
    ...
    formal_methods = [k_same_net, k_same_pixel]#[k_same_net, k_same_pixel] # , k_same_M, k_same_net

    src_path = "./tea/"
    src_path_aam = "./folds_aam/"
    dst_path = "./deid/"
    cluster_cache = "./tea_cluster_cache/"
    folds = ["XM2VTS_224_60", "RAFD_244"]

    for fold in folds:

        if not os.path.exists(os.path.join(dst_path, fold)):
            os.makedirs(os.path.join(dst_path, fold))
    ...
    do_naive = False #True
    do_formal = True #True # TODO: test k_same_M
